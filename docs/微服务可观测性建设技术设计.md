...
**⚠️ 抓取侧的标签口径（铺集群前必须先定，否则数据采回来也不能用）**：`spec/metrics-format.md` 的四个通用 label（`service` / `env` / `instance` / `community`）是**服务自己暴露**出来的，而 Prometheus 抓取时还会附加一批**服务端 label** —— `job`、`instance`（**默认就是抓取地址 `__address__`**）、以及 SD / relabel 打上的 label。默认 `honor_labels: false` 下**服务端 label 获胜**，服务暴露的那个被改名成 `exported_*`。其中 `instance` 是**必然**被顶掉的一个：不需要写任何 relabel，Prometheus 天生就会占用它，结果是**日志里的 `instance`（部署实例名）与指标里的 `instance`（pod IP）同名不同义，日志与指标再也无法用 `instance` 关联** —— 而这正是「QPS / 延时」大盘最常用的下钻维度。

**故 Agent 侧所有 job 一律 `honor_labels: true`**，并配套三条纪律：`instance` 让给 SDK、抓取侧要保留目标地址就另起 `pod_ip`；**不要**把 `__meta_kubernetes_service_name` 映射成 `service`（与 SDK 的 `service` 正面冲突）；`namespace` / `pod` 可以照打 —— `honor_labels` 是**逐 label** 生效的，不冲突就保留，这正是「`instance` 归 SDK、`namespace`/`pod` 归抓取侧」的组合。细则与实测输出见 `spec/metrics-format.md`「抓取侧的 label 冲突」。

这个坑在 KSM job 上先撞了一次：83 个 Pod 的 `kube_pod_info` 全被贴成 `namespace="monitoring"`、真实命名空间被挤到 `exported_namespace`，`kube_pod_info{namespace="review"}` 静默查空（修见 infra-common#4272）。**服务抓取 job 上 `instance` 是同一机制的必然复现** —— 所以在脚本化铺其余集群**之前**先把口径写死进模板。
...
| **标签口径** | ⚠️ **所有 job 一律 `honor_labels: true`** —— 否则 Prometheus 的服务端 label 会顶掉 SDK 打的 `service/env/instance/community`（`instance` 必被 `__address__` 顶掉，日志与指标的 `instance` 从此同名不同义）。配套三条：`instance` 让给 SDK、抓取地址另起 `pod_ip`；不要把 k8s Service 名映射成 `service`；`namespace` / `pod` 可照打。细则见 `spec/metrics-format.md`「抓取侧的 label 冲突」（§4.7.1） |
...
### 4.11 Grafana 认证与访问（SSO）

**现状**：只有一个 `admin` 账号，口令存 Vault（§4.9.1）；`GF_USERS_ALLOW_SIGN_UP=false`，匿名访问未开。

**读者分三层，本节只解决中间那层**：

| 读者 | 想看什么 | 形态 | 依据 |
| --- | --- | --- | --- |
| 运维（深度排障） | **为什么**坏：QPS / 错误率 / 延迟 / 资源 + 日志下钻 | 本节：Grafana + SSO | §4.7.4 |
| **研发 / 社区运营（日常看板）** | 服务与社区健康度 | 本节：Grafana + SSO，同一实例按身份区分权限 | §4.7.4 |
| 社区用户 / 外部开发者 | **哪块**坏了 | **状态页，不是 Grafana** | §4.7.5 |

> ⚠️ **对外那一层不进 Grafana**——不是「先不做」，是 **Grafana OSS（开源版）的能力不足以安全地对外**（理由见 §4.11.2）。状态页仍按 §4.7.5 与 Grafana 解耦。

#### 4.11.1 方案：GitHub OAuth

**选 GitHub 而非 Gitee**：Grafana **内建** GitHub provider（OSS 版可用，v2.0+），配置全是声明式的；**Gitee 没有内建 provider**，只能走 Generic OAuth，要手写 `auth_url` / `token_url` / `api_url` 加 JMESPath 字段映射，且 Gitee 的 API 结构与 GitHub 不同——成本翻倍、踩坑面更大。社区研发本来就有 GitHub 账号，无新建账号成本。

**改动全在 values，chart 一行不动**：`charts/grafana` 的 `env` 是通用 k8s EnvVar 列表（`GF_*` 覆盖 `grafana.ini` 全部配置项），`secrets` 是列表驱动的 `{name, path, keysMap}`。改动仅限 `common/grafana/prod/values.yaml`（及 dashboards values，如需）。

| 配置 | 值 | 说明 |
| --- | --- | --- |
| `GF_AUTH_GITHUB_ENABLED` | `true` | 开关 |
| `GF_AUTH_GITHUB_CLIENT_ID` | OAuth App 的 client id | 明文，进 values |
| `GF_AUTH_GITHUB_CLIENT_SECRET` | — | **从 Vault 注入**（`secretKeyRef` → 新增 `community-grafana-oauth`），**不进 values** |
| `GF_AUTH_GITHUB_ALLOWED_ORGANIZATIONS` | `opensourceways` | ⚠️ **不配等于把 Grafana 白送**（任何 GitHub 用户都能登录并自建账号） |
| `GF_AUTH_GITHUB_SCOPES` | `user:email,read:org` | ⚠️ **缺 `read:org`，上面的组织限制评估不了、形同虚设** |
| `GF_AUTH_GITHUB_ROLE_ATTRIBUTE_PATH` | `'Viewer'` | JMESPath；初期统一 Viewer |

**OAuth App 的 callback URL 必须带子路径**：`https://monitoring.osinfra.cn/grafana/login/github`。漏掉 `/grafana`（Grafana 跑在子路径下，§4.9.1）的症状是**回调 404**。

#### 4.11.2 OSS 版的能力边界（决定了本节范围）

| 能力 | Grafana OSS（开源版） | 说明 |
| --- | --- | --- |
| GitHub / GitLab / Google / Generic OAuth | ✅ | 自建镜像即 OSS 版 |
| `role_attribute_path` 角色映射 | ✅ | 只能映射**角色**（Viewer / Editor / Admin / GrafanaAdmin），**不能映射 Team** |
| **Team Sync**（按 GitHub team 同步 Grafana team） | ❌ **Enterprise** | ⇒ **「按社区自动分权」做不到**，要手工维护 Team 成员 |
| **SAML** | ❌ Enterprise | 若将来要接企业内部 IdP，是采购问题、不是配置问题 |
| **细粒度数据源权限** | ❌ Enterprise | ⇒ **开源版里 Viewer 可对数据源发任意查询**，不限于面板上定义的那些 |

> ⚠️ **最后一条是「不能对社区用户开放 Grafana」的技术根据**：拿到 session 后可直接打 `/api/ds/query` 发任意 PromQL，绕过面板权限——Grafana 视其为既定行为。叠加两件事后风险更大：
> 1. 现有 series 带 `namespace` / `pod` / `pod_ip` / `cluster`，公开即等于公开内部资产清单；
> 2. 数据源用的是 Prometheus 的 `grafana` 用户，而 **Prometheus 没有只读角色，该口令同样能 POST `/api/v1/write`**（该限制已在 Vault 记录中声明接受）——公开 Grafana 等于公开「往中心 TSDB 写」的能力，而 WAF 是它前面唯一一层。

#### 4.11.3 安全要点

1. **`ALLOWED_ORGANIZATIONS` 必配**，且 `SCOPES` 带 `read:org`（见 §4.11.1）——否则任何 GitHub 用户都能登录，比现状更糟。
2. **组织侧若开了「OAuth App 访问限制」**，该 App 还需 org owner 单独批准一次——**这是联调阶段最容易卡住的一步**。
3. **`admin` 账号必须保留**作 break-glass：OAuth 配错导致全员登不进来时，它是唯一入口。口令继续走 Vault（§4.9.1）。
4. **初期统一给 Viewer**（`role_attribute_path` 固定返回 `'Viewer'`），不做分权——Team Sync 是 Enterprise（§4.11.2），自动同步做不了。
5. 改动生效会**触发一次 Pod 重启**（`strategy: Recreate`，因 PVC 是 RWO，§4.9.1），有数十秒不可用。

#### 4.11.4 工作量

| 项 | 人天 | 承担方 |
| --- | --- | --- |
| values 改动 + PR | 0.5 | 本方案 |
| 注册 GitHub OAuth App（callback 带子路径） | 0.1 | **需 opensourceways org owner** |
| Vault 写入 `client_secret` | 0.1 | 运维 |
| ArgoCD sync + 联调（登录 / 回调 / 组织限制 / 角色） | 0.5 | 本方案 |
| 文档与 break-glass 交接 | 0.25 | 本方案 |
| **合计** | **≈1～1.5 人天** | 日历 **2～3 天**（卡在 org owner 与运维） |

#### 4.11.5 明确不做

- **不对社区用户 / 外部开发者开放 Grafana**（含匿名访问与任何形式的公开分享）——理由见 §4.11.2；对外走 §4.7.5 的状态页。
- **不做按社区自动分权**——Team Sync 是 Enterprise（§4.11.2）。
- **不接 SAML / 企业内部 IdP**——同上，属采购范畴。
