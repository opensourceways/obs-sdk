...
- ⚠️ **不照搬资源块的公网明文 HTTP**——资源块当前是 `http://<EIP>:9090` + Basic Auth，而其 TLS 清单（`prometheus-remote-write-certificate` / `-ingress`）在 `kustomization.yaml` 中**被注释、标为 Phase 2**。本方案**搭建时就要上 HTTPS**（§4.7.1），不留这个尾巴。
